...
from mlmc.mc import asian_price_mc
np.random.seed(42)
def main():
    S0 = 100.0
    r = 0.05
    ...
    print("Asian reference:", price_ref, "SE:", se_ref)
    ...
    for idx, step in enumerate(test_steps):
        np.random.seed(42)
        price, se = asian_price_mc(S0, mu, sigma, step, n_paths_test, K, r, T)
    ...
    plt.title("Asian call: discretization bias vs time step")
    ...
    plt.xlabel("n_steps")
    plt.ylabel("price")
    plt.title("Asian call: price vs n_steps (with MC SE)")
    plt.grid(True)
    plt.show()
