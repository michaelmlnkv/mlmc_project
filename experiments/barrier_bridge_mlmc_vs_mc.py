...
def main():
    ...
    mu = r
    max_level = 10
    epsilons = [0.2, 0.1, 0.05, 0.025, 0.0125, 0.00625]
    n_paths0 = 20000
    barrier = 120
    warmup()
    ...
    for idx, eps in enumerate(epsilons):
        #MLMC benchmark
        max_level = int(np.ceil(np.log2(2*T/eps)))
        print(max_level)
        start =  time.perf_counter_ns()
        price, se = mlmc_barrier(S0, mu, sigma, max_level, K, barrier, r, T, eps, bridge=True)
        ...
        if idx != 6:
            ...
            n_steps = 2**max_level
            start = time.perf_counter_ns()
            mean0, se0 = barrier_price_mc(S0, mu, sigma, n_steps, n_paths0, K, barrier, r, T, bridge=True)
            n_paths = int(np.ceil(n_paths0 * (2*se0/eps)**2))
            price1, se1 = barrier_price_mc(S0, mu, sigma, n_steps, n_paths, K, barrier, r, T, bridge=True)
            end = time.perf_counter_ns()
    ...
    # Reference slopes
    c_mc = mc_costs[1] * eps_arr[1] ** 3
    c_mlmc = mlmc_costs[1] * eps_arr[1] ** 2
